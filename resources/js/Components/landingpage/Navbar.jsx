...
export default function Navbar({ data, className }) {
    ...
    return (
        <header
            className={`flex items-center h-14 md:h-full w-full  justify-between md:justify-center z-20 backdrop-blur-lg shadow-lg top-0 px-5 sticky bg-opacity-60 ${className}`}
        >
            <h3 className="font-bold text-xl md:hidden">Skensa</h3>
            <nav
                className="fixed md:relative flex md:max-w-[90%] mx-auto flex-col justify-center md:flex py-4 flex-wrap items-center gap-1 w-full h-screen md:h-full backdrop-blur-lg md:backdrop-blur-none bg-slate-950 md:bg-transparent  bg-opacity-60 left-0 -top-[200vh] md:top-0  md:flex-row  duration-1000"
                ref={navRef}
            >
                {data &&
                    data.map((dat, idx) => {
